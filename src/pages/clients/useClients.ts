...
function toRow(c: BackendClient, stats?: { total: number; next: string | null }): ClientRow {
  return {
    id: String(c.id),
    ...
    (async () => {
      try {
        if (isOrg) {
          ...
          if (cancelled) return;
          setRows(res.data.map((c) => toRow(c)));
          setTotalPages(res.pagination.totalPages);
          setTotal(res.pagination.total);
        } else if (user) {
          const myTherapistId = await getMyTherapistId();
          const [{ assignedClients, appointmentClients }, myAppointments] = await Promise.all([
            getMyClients(),
          ...
          let list = deduped.map((c) => toRow(c, statsByClient.get(String(c.id))));
